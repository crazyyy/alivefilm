...
function wpeFootNav() {
  ...
  array(
    ...
    'link_before'     => '',
    'link_after'      => '',
    'items_wrap'      => '<ul class="modalnav">%3$s</ul>',
    'depth'           => 0,
    'walker'          => ''
    )
  );
...
function dt_add_main_js(){
  ...
  wp_register_script( 'main-js', get_template_directory_uri() . '/js/scripts.js', array( 'jquery' ), '1.0', false );
...
function load_more_func() {
  ...
  if ($posts_query->have_posts()) {
    ...
    $i = 0; while ( $posts_query->have_posts() ) : $posts_query->the_post(); ?>
